...
parser.add_argument('--data_dir', type=str, 
                    default='./datasets/anomaly/SMD/processed', help='Location of datasets.')
parser.add_argument('--output_dir', type=str, 
                    default='./checkpoint/model')
parser.add_argument('--name',default='GANF_SMD')
# restore
...
for _ in range(max_iter):
    ...
    while rho < rho_max:
        ...
        for _ in range(args.n_epochs):
            ...
            print(val_loader.dataset.label)
            #roc_val = roc_auc_score(np.asarray(val_loader.dataset.label,dtype=int),loss_val)
            ...
            print('Epoch: {}, train -log_prob: {:.2f}, test -log_prob: {:.2f}, roc_val: {:.4f}'
...
for _ in range(30):
    ...
    #print('Epoch: {}, train -log_prob: {:.2f}, test -log_prob: {:.2f}, roc_val: {:.4f}, roc_test: {:.4f} ,h: {}'\
    #        .format(epoch, np.mean(loss_train), np.mean(loss_val), roc_val, roc_test, h.item()))
    print('Epoch: {}, train -log_prob: {:.2f}, test -log_prob: {:.2f}'
          .format(epoch, np.mean(loss_train), np.mean(loss_val), h.item()))
